...
const OrderLog = () => {
    const { data: orderList, isLoading: isOrderListLoad } = useQuery(['order-list'],
        () => {
            ...
            let queryParams = [];
            queryParams.push(`payment.status=${config.paymentStatus[0]}`);
            ...
            <section className='main-wrap'>
                <Container>
                    ...
                    <Row className="mt-5">
                        ...
                        <Col lg={10} md={9}>
                            {isOrderListLoad ? <Loader /> : orderList?.orders?.length == 0 ? <NoDataFound msg={"No Data Found"} /> : <OrderList log={true} orderList={orderList?.orders} />}
                        </Col>
                    </Row>
                </Container>
            </section>
